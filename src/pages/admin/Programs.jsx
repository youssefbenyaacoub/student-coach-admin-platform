import { Plus, RefreshCcw, Trash2, Edit, Search, Calendar, Users, Briefcase } from 'lucide-react'
...
export default function AdminPrograms() {
  ...
  // Data mapping
  const allPrograms = useMemo(() => data?.programs ?? [], [data?.programs])
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-lg text-slate-500 font-medium">Create and manage cohorts.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm dark:bg-slate-800 dark:border-slate-700">
          ...
          <div className="text-2xl font-heading font-bold text-slate-900 dark:text-white">{allPrograms.length}</div>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm dark:bg-slate-800 dark:border-slate-700">
          <div className="flex items-center gap-3 mb-2">
          ...
          <div className="text-2xl font-heading font-bold text-slate-900 dark:text-white">
            {allPrograms.reduce((acc, p) => acc + (p.participantStudentIds?.length || 0), 0)}
          </div>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm dark:bg-slate-800 dark:border-slate-700">
          <div className="flex items-center gap-3 mb-2">
          ...
          <div className="text-2xl font-heading font-bold text-slate-900 dark:text-white">
            {allPrograms.filter(p => getComputedStatus(p) === 'ongoing').length}
          </div>
        </div>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1.5 dark:text-slate-300">Resources (JSON)</label>
            <textarea
              ...
              placeholder='[{"title":"BMC","url":"..."}]'
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 border rounded-xl dark:border-slate-700">
              <div className="text-sm font-bold mb-2">Coaches</div>
              <div className="space-y-1 max-h-32 overflow-auto">
                {coaches.map(c => (
                  <label key={c.id} className="flex items-center gap-2 text-xs">
                    <input
                      type="checkbox"
                      checked={(form.coachIds || []).includes(c.id)}
                      onChange={(e) => {
                        const next = new Set(form.coachIds || [])
                        if (e.target.checked) next.add(c.id)
                        else next.delete(c.id)
                        setForm(f => ({ ...f, coachIds: Array.from(next) }))
                      }}
                    />
                    {c.name}
                  </label>
            ...
            <div className="p-3 border rounded-xl dark:border-slate-700">
              <div className="text-sm font-bold mb-2">Students</div>
              <div className="space-y-1 max-h-32 overflow-auto">
                {students.map(s => (
                  <label key={s.id} className="flex items-center gap-2 text-xs">
                    <input
                      type="checkbox"
                      checked={(form.participantStudentIds || []).includes(s.id)}
                      onChange={(e) => {
                        const next = new Set(form.participantStudentIds || [])
                        if (e.target.checked) next.add(s.id)
                        else next.delete(s.id)
                        setForm(f => ({ ...f, participantStudentIds: Array.from(next) }))
                      }}
                    />
                    {s.name}
                  </label>
      ...
      <ConfirmDialog
        ...
        description="This will permanently remove the program."
        confirmLabel="Delete"
        isDanger
